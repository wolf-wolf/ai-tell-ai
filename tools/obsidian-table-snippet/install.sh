#!/usr/bin/env bash
# 将表格字号 CSS snippet 安装到 Vault 的 .obsidian/snippets/ 并尝试自动启用
set -euo pipefail
...
PY
  ...
  echo "Tip: enable manually in Settings → Appearance → CSS snippets → ai-table"
fi

echo ""
echo "Installed to: $DEST"
echo "Next: Obsidian → Settings → Appearance → CSS snippets → reload → enable ai-table"
