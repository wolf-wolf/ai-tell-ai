#!/usr/bin/env bash
# 将行内 code CSS snippet 安装到 Vault 的 .obsidian/snippets/ 并尝试自动启用
set -euo pipefail
...
SNIPPET_FILE="ai-inline-code.css"
DEST="$SNIPPETS_DIR/$SNIPPET_FILE"
SNIPPET_ID="${SNIPPET_FILE%.css}"

mkdir -p "$SNIPPETS_DIR"
cp "$SCRIPT_DIR/inline-code.css" "$DEST"
...
PY
  ...
  echo "Tip: enable manually in Settings → Appearance → CSS snippets → ai-inline-code"
fi

echo ""
echo "Installed to: $DEST"
echo "Next: Obsidian → Settings → Appearance → CSS snippets → reload → enable ai-inline-code"
