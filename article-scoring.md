...
| 为凑字数对称扩各节、应拆不拆 | 篇幅随机制需要；>12k 默认拆或 outline 说明不拆理由 | `writing-rules.md` 篇幅软指引 |
...
| 概念 | 值 |
| --- | --- |
| 不采用 | 0–100 总分、字数、阅读时间（**不作评分维**） |
| 篇幅软指引 | 写前 scope 估算见 `writing-rules.md`「篇幅软指引（非 KPI）」；**不进入 `done` 公式** |
| 深度 | Q1（透彻 + **条件性因果**） |
