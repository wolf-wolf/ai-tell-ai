...
本仓库**不使用**正文元数据表（字数 / 阅读时间 / 难度表）。修订日期写在 frontmatter `updated`；需要标注难度时用可选字段 `difficulty`（如 `⭐⭐⭐` 或 `3/5`），与 [map.md](../../../map.md) 稳定性标签分工：`stability` 表时效，`difficulty` 表阅读门槛（可选）。写前 scope 估算见 [writing-rules.md — 篇幅软指引（非 KPI）](../../../writing-rules.md#篇幅软指引非-kpi)（**非评分项**，不进入 `article-scoring.md` 的 `done`）。
...
`writing-rules.md` 不固定标题；**先定主类型**（见 [文章类型与内容契约](../../../writing-rules.md#文章类型与内容契约)），再按下表选脊柱。常见组合：

| 主类型 | 常见 `##` 示例 |
| --- | --- |
| 概念与范式 | 问题语境、整体模型、局部概念分工、实践含义 |
| 技术与机制 | 定义与直觉、形式化与复杂度、实现要点、与相关算法对比 |
| 框架与项目 | 架构与核心机制、实现要点 / Prompt 契约、使用方式、选型对比 |
| 方法论与治理 | 问题语境、操作框架、落地检查单、反模式 |
| 数据工程 | 来源与适用、清洗/标注规范、质量门禁、血缘与版本 |
| 速览与地图 | 主题边界、分域导航、推荐阅读顺序 |
