...
> [!tip] 核心本质
> RLHF 用人类偏好训练奖励模型，再以强化学习把策略推向「更像人愿看到的回答」——弥补预训练只优化「下一 token 连贯」与「对人有用」之间的鸿沟。若没有对齐，模型会续写得像文本，却不听指令、不安全或过度端水；但奖励模型不完美时，优化会放大偏置与奖励黑客（见 [[rlhf-bias]]）。

## 生命周期与演进

**当前定位**：对齐范式的经典教科书流程（SFT → RM → PPO）；工业界大量转向 DPO/ORPO 等无显式 RM 的变体。

**预期寿命**：「用偏好信号对齐」长期存在；RLHF 三阶段工程占比下降。

**近期演进**：RLAIF、Constitutional AI、因果奖励建模、信息瓶颈 RM 等缓解偏置。

**终极威胁**：可验证奖励（RLVR）、环境反馈或自动化评判若更可靠，人类标量 RM 角色收缩。
...
- [[llm]] — 预训练、SFT 与 RLHF 的完整上下文
- [[rlhf-bias]] — 奖励偏置与缓解策略
- [[rlvr]] — 可验证奖励等替代对齐信号
- [InstructGPT（Ouyang et al., 2022）](https://arxiv.org/abs/2203.02155) — RLHF 原始工程流程
- [DPO（Rafailov et al., 2023）](https://arxiv.org/abs/2305.18290) — 绕过显式 RM 的主流替代
- [Constitutional AI（2022）](https://arxiv.org/abs/2212.08073) — RLAIF 思路
- [Reward Hacking（Skalse et al., 2022）](https://arxiv.org/abs/2209.13085) — 失败模式综述
