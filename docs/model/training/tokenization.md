...
**你让模型数一数「strawberry」里有几个字母 r，它答错了——为什么？**

*因为模型从来没有「看」过这个单词的字母，它只看到了一个叫 `strawberry` 的 token，内部没有字母级别的结构。*

这件事说明：模型处理的不是文字，不是字符，而是 token。在文本进入模型之前，tokenization 这一步就已经决定了模型能感知到什么、感知不到什么。
...
> （占位符——写下你自己第一次搞清楚 token 边界时的顿悟，或者你发现哪个任务因为 tokenization 而变得奇怪）
