...
> [!tip] 核心本质
> **仅保证结构**时，目标只有两件事：`JSON.parse` 成功，且输出符合你定义的 **required 字段、类型与 enum**（字段完整）。这靠 **JSON Schema + 约束解码**（或 API 的 strict / `response_schema`）在生成时屏蔽非法 token，而不是靠 prompt 里的「请只输出 JSON」——后者在复杂 schema 下仍有约 **8%–20%** 结构失败（见 [[#方法谱系与有据效果区间|§方法谱系]]）。**语义对不对**（值是否事实正确）是另一层，本文 [[#仅要结构可靠（可解析 + 字段完整）|§仅要结构可靠]] 以下的长文也覆盖语义与基准，若你只关心可解析与字段完整，读完该节即可选型。
...
- **工程权衡**：结构失败从「常见」变为「罕见」，但应用 eval 必须同时看**业务指标**是否下降；必要时用 [[#7. 流程拆分：先推理后格式化（降低「格式税」）|§7]] 双阶段或放宽约束字段（先粗后细）。
