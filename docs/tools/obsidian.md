...
- 路径型链接（`[[folder/note]]`）与仅文件名链接（`[[note]]`）取决于你的「文件与链接」设置；大库更常统一一种风格
...
多人或多源笔记合并时，Obsidian 提供**冲突可见**的文本 diff 与链接修复面，而不是黑盒数据库——与 [[knowledge-fusion]] 所关心的「多源对齐」在载体层可以配合，但合并规则仍须你自己或工具链定义。
